	...
	public Register() {
		...
		btnNewButton_1.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				int row=addCustZL();
				if (row > 0) {
					JOptionPane.showMessageDialog(Register.this, "添加完成");
					// 推荐更新成功后，刷新一下数据，也就是再查询一下
//					searchCustZL(Integer.parseInt(textField1.getText()));
				} else {
					JOptionPane.showMessageDialog(Register.this, "添加出错！请检查数据");
				}
			}
		});
...
//				}
		...
		btnNewButton_4.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				//new CommonDAOImpl().rollbackBatch();
			}
		});
        ...
        ImageIcon background1 = new ImageIcon("image/3.png");  
...
//		JButton bu=new JButton("F2");
	...
	private void initData() {
		...
		loadDept1();
		loadDept2();
//		comboBox_2.addItem(sdf.format(new Date()));
		//loadDept4();
		loadDept5();
//		comboBox1.setSelectedIndex(0);
	...
	private void loadDept1() {
		//List<Dept> deptList = new DeptDAO().findAll();
		List<Customer> List = new CommonDAOImpl().executeQuery2(Customer.class, "SELECT * FROM user_customer", null);
		for (Customer c : List) {
			comboBox1.addItem(c.toString());
		}
	}
	private void loadDept2() {
		List<String> List = new CommonDAOImpl().executeQuery2(String.class, "SELECT Car_Id FROM tb_car", null);
		for (String s : List) {
	...
	private void loadDept4() {
		List<CustZL> List = new CommonDAOImpl().executeQuery2(CustZL.class, "SELECT * FROM tb_cust_zl", null);
		for (CustZL d : List) {
			comboBox_3.addItem(d.toString1());
		}
	}
	private void loadDept5() {
		List<String> List = new CommonDAOImpl().executeQuery2(String.class, "SELECT name FROM driver", null);
		for (String s : List) {
	...
	private void searchCustZL(int id) {
		...
		params.add(id);
		List<CustZL> czlList = new CommonDAOImpl().executeQuery2(CustZL.class, "SELECT * FROM tb_cust_zl WHERE Inner_Id=?", params);
		if (czlList.size() > 0) {
			...
			JOptionPane.showMessageDialog(Register.this, "没有这个员工");
		}
	}
